...
Deno.serve(async (req) => {
  ...
  // 1. Auth admin — ⚠️ cette Edge expose les manager_token (tokens responsables) : token admin OBLIGATOIRE.
  ...
  // 2. Liste des équipes (triée nom_equipe).
  const { data: rData, error: rErr } = await supabase.rpc("equipes_liste");
  if (rErr) return json({ ok: false, error: rErr.message || "Echec de la lecture des equipes" }, 500);

  return json({ ok: true, admin: adminNom, equipes: rData ?? [] });
});
