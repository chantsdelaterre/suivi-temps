...
Deno.serve(async (req) => {
  ...
  if (!admin_token || id == null) return json({ ok: false, error: "admin_token et id requis" }, 400);
  ...
  // 2. Appel de la RPC métier (un seul param : id). Le RAISE (contrat non à venir) remonte via rErr.message.
  const { data: rData, error: rErr } = await supabase.rpc("supprimer_contrat", { p_id: id });
  if (rErr) return json({ ok: false, error: rErr.message || "Echec de la suppression" }, 500);
